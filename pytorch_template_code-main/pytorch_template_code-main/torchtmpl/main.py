...
def train(config):
    ...
    input_size = (batch_size, 3, 224, 224)  # Adapté pour l'entrée
    num_classes = model_config["num_classes"]  # Même si c'est une régression, vous pouvez garder un champ ici
    model = build_model(model_config, input_size=input_size, num_classes=num_classes)
    model.to(device)

    # Configure la fonction de perte pour la régression
    ...
    # Enregistrement des logs et des répertoires
