...
class Autopilot(Node):
    def __init__(self):
        ...
        # Load the ONNX model
        try:
            self.model = onnxruntime.InferenceSession("/home/profil/monkade1u/hammou/iliar_solution2/src/iliar_solution2/dummy_model.onnx")
            self.get_logger().info("ONNX model loaded successfully.")
        except Exception as e:
            self.get_logger().error(f"Failed to load ONNX model: {e}")

        self.sub_left = Subscriber(self, CompressedImage, "/left", qos_profile=qos_profile)
        self.sub_front = Subscriber(self, CompressedImage, "/forward", qos_profile=qos_profile)
        self.sub_right = Subscriber(self, CompressedImage, "/right", qos_profile=qos_profile)

        self.steering_pub = self.create_publisher(Float64, "/steer", 10)
